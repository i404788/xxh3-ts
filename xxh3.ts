...
function XXH3_scrambleAcc(acc: BigUint64Array, key: Buffer) {
    for (let i = 0; i < ACC_NB; i++) {
        ...
        const right = 2 * i + 1;
        acc[i] ^= acc[i] >> 47n;
        const p1 = XXH_mult32to64((acc[i] & 0xFFFFFFFFn), n(key.readUInt32LE(left)));
        const p2 = XXH_mult32to64(acc[i] >> 32n, n(key.readUInt32LE(right)));
        acc[i] = p1 ^ p2;
    }
}
...
function XXH3_len_1to3_128b(data: Buffer, key32: Buffer, seed: bigint) {
    ...
    assert(len > 0 && len <= 3);
    {
        ...
        return (XXH3_avalanche(ll11 & mask64) << 64n) | XXH3_avalanche(ll12 & mask64)
    }
}
...
function XXH3_len_4to8_128b(data: Buffer, key32: Buffer, seed: bigint) {
    ...
    assert(len >= 4 && len <= 8);
    {
        ...
        return (XXH3_avalanche(acc1 & mask64) << 64n) | XXH3_avalanche(acc2 & mask64)
    }
}
...
function XXH3_len_0to16_128b(data: Buffer, seed: bigint) {
    ...
    assert(len <= 16);
    {
        ...
        return (seed << 64n) | seed;
    }
}

// 16 byte min input
export function XXH3_128(data: Buffer, seed: bigint = 0n) {
